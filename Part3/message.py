...
def summarize_to_one_sentence(paragraph):
    ...
    summarized_sentence = summary[0]._text
...
def fetch_definition(domain):
    # Mock function to generate a placeholder definition
    return f"This is a placeholder definition for {domain}."
...
def ask_gpt2(function):
        client2 = Client()
        messages2 = [
            {"role": "system",
             "content": "You are attempting to write a short-one sentence long definition about a given java function."},
            {"role": "system",
             "content": "As you guess this, this should be a short one sentence string."},
            {"role": "system",
             "content": "Answer in a regular grammatically correct sentence."
             }
        ]
        # Construct the prompt with the object description and option descriptions
...
def getFunc(function):
        gpt_response2 = ask_gpt2(function)
        answer2 = ""
        for chunk in gpt_response2:
            if chunk.choices[0].delta.content:
                answer2 += (chunk.choices[0].delta.content.strip('*') or "")
        initial_sentence = answer2.split(".")[0] + "."
            ...
            if domain in domain_functions:
                for class_name, functions in domain_functions[domain].items():
                    ...
                    for function in functions:
                        ...
                        print(f"    Function call: {function}")
                        print(f"    Definition: {getFunc(function)}")
                    print()  # Add a newline after each class's functions
                print()  # Add a newline after each domain's classes
